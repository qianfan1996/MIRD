...
class MAG(nn.Module):
    def __init__(self, hidden_size, beta_shift, dropout_prob, acoustic_dim=33, visual_dim=709, text_dim=768):
    ...
    def forward(self, text_embedding, visual, acoustic):
        ...
        hm_norm_ones = torch.ones(hm_norm.shape, requires_grad=True).to(get_device())
        hm_norm = torch.where(hm_norm == 0, hm_norm_ones, hm_norm)

        thresh_hold = (em_norm / (hm_norm + eps)) * self.beta_shift

        ones = torch.ones(thresh_hold.shape, requires_grad=True).to(get_device())
...
class WA(nn.Module):
    def __init__(self, hidden_size, beta_shift, dropout_prob, acoustic_dim=33, visual_dim=709, text_dim=768):
        ...
        self.W_v_ = nn.Linear(visual_dim, text_dim)
        self.W_a_ = nn.Linear(acoustic_dim, text_dim)
