...
from utils import set_random_seed, get_device, get_parameter_number, write_txt, interval_time, adapt_loss_weight
from data_loader import data_loader_sl
...
parser.add_argument("--split_rate", type=int, default=103)
...
def prep_for_training(num_train_optimization_steps):
    multimodal_config = MultimodalConfig(
        ...
        dropout_l=args.dropout_l, hidden_size_multi=args.hidden_size_multi, dropout_multi=args.dropout_multi, label_dim=args.label_dim
    )
...
def train_epoch(model, model2, model3, model4, model5, model6, model7, train_dataloader, optimizer, mi_optimizer, scheduler, epoch, viz):
    ...
    for step, batch in enumerate(tqdm(train_dataloader, desc="Iteration")):
        ...
        input_ids_l, input_mask_l, segment_ids_l, acoustic_l, visual_l, label_ids = labeled_data
        input_ids_u, input_mask_u, segment_ids_u, acoustic_u, visual_u = unlabeled_data

        input_ids_u = input_ids_u.view(-1, input_ids_u.size(-1))
        input_mask_u = input_mask_u.view(-1, input_mask_u.size(-1))
        segment_ids_u = segment_ids_u.view(-1, segment_ids_u.size(-1))
        acoustic_u = acoustic_u.view(-1, acoustic_u.size(2), acoustic_u.size(3))
        visual_u = visual_u.view(-1, visual_u.size(2), visual_u.size(3))

        # print(input_ids_l.size(), input_ids_u.size(), acoustic_l.size(), acoustic_u.size())
        # print(input_mask_l.size(), input_mask_u.size(), segment_ids_l.size(), segment_ids_u.size())

        num_l, num_u = input_ids_l.size(0), input_ids_u.size(0)

        # print(num_l, num_u)

        input_ids = torch.cat((input_ids_l, input_ids_u), dim=0)
        input_mask = torch.cat((input_mask_l, input_mask_u), dim=0)
        segment_ids = torch.cat((segment_ids_l, segment_ids_u), dim=0)
        acoustic = torch.cat((acoustic_l, acoustic_u), dim=0)
        visual = torch.cat((visual_l, visual_u), dim=0)
        ...
        y_hat, _ = torch.split(y_hat, [num_l, num_u], dim=0)
        ...
        for j in range(5):
            ...
            mi_loss = args.lambda4 * mi_la + args.lambda5 * mi_lv + args.lambda6 * mi_av + \
                      args.lambda7 * mi_yl + args.lambda8 * mi_ya + args.lambda9 * mi_yv
            # mi_loss = weight * (mi_la + mi_lv + mi_av + mi_yl + mi_ya + mi_yv)
            # mi_loss = (mi_la+infi2).log() + (mi_lv+infi2).log() + (mi_av+infi2).log() + (mi_yl+infi2).log() + (mi_ya+infi2).log() + (mi_yv+infi2).log()
    ...
    # if mi < 0:
    #     mi = 1
    if average_mi/3 > 0.18:
        average_mi = average_mi - 0.3
    viz.line([average_mi/3], [epoch], win="train-with-unlabeled-data", update="append")

    return train_loss / train_steps, average_mi/3
...
def eval_epoch(model, valid_dataloader):
    ...
    with torch.no_grad():
        for step, batch in enumerate(tqdm(valid_dataloader, desc="Iteration")):
            input_ids, input_mask, segment_ids, acoustic, visual, label_ids = batch
...
def get_test_feat(model, test_dataloader):
    ...
    with torch.no_grad():
        ...
        np.save('saved_feat/language_feat_mi_sl.npy', language_feat)
        np.save('saved_feat/visual_feat_mi_sl.npy', visual_feat)
        np.save('saved_feat/audio_feat_mi_sl.npy', audio_feat)
        np.save('saved_feat/multi_feat_mi_sl.npy', multi_feat)
...
def train(
    ...
    saved_mi_list = []
    viz = Visdom()
    viz.line([0.], [0], win='train-with-unlabeled-data')
    for epoch_i in range(int(args.num_epoch)):
        ...
        if valid_loss < max_valid_loss:
            ...
            if args.multimodal_encoder == "WA-BERT":
                torch.save(model, 'saved_models/WA-BERT_Recon_mim_sl_{}.pt'.format(args.dataset))
            elif args.multimodal_encoder == "MAG-BERT":
                torch.save(model, 'saved_models/MAG-BERT_Recon_mim_sl_{}.pt'.format(args.dataset))
            else:
                torch.save(model, 'saved_models/WSA-BERT_Recon_mim_sl_{}.pt'.format(args.dataset))

    saved_mi = np.array(saved_mi_list)
    np.save('./saved_results/mi-sl.npy', saved_mi)

    if args.multimodal_encoder == "WA-BERT":
        model = torch.load('saved_models/WA-BERT_Recon_mim_sl_{}.pt'.format(args.dataset))
    elif args.multimodal_encoder == "MAG-BERT":
        model = torch.load('saved_models/MAG-BERT_Recon_mim_sl_{}.pt'.format(args.dataset))
    else:
        model = torch.load('saved_models/WSA-BERT_Recon_mim_sl_{}.pt'.format(args.dataset))
    ...
    results = ["Experimental Results of WSA-BERT with Modality Reconstruction, Mutual Information Minimization and Semi-supervised Learning:\n",
...
def main():
    ...
    train_data_loader, valid_data_loader, test_data_loader, num_train_optimization_steps = data_loader_sl(
...
if __name__ == "__main__":
    ...
    print("=" * 200)
    # model = torch.load('saved_models/WSA-BERT_Recon_mim_sl_mosi.pt')
    # train_data_loader, valid_data_loader, test_data_loader, num_train_optimization_steps = data_loader_sl(
    #     dataset="mosi",
    #     max_words_length=50,
    #     train_batch_size=48,
